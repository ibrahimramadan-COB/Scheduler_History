"""
scripts/snowflake_writer_v2.py — the NEW, event-based writer, running
alongside (not replacing) snowflake_writer.py. Targets
SCHEDULER_HISTORY_DATA2: delete-and-replace PER EVENT (Date + Type +
Patient + Case + Clinic + Calendar Name) instead of per-row MERGE, and
also captures CHANGED_FROM / CHANGED_TO.

This table is still being validated for accuracy — the old table
(via snowflake_writer.py) remains the mandatory, relied-upon daily
source until this one is confirmed reliable enough to take over.

Usage: python snowflake_writer_v2.py
Reads RAW_FILE from env (same as snowflake_writer.py).
"""
...
def load_to_snowflake(conn, df: pd.DataFrame, run_id: str):
    ...
    try:
        ...
        print(f"   [v2] 📤 Staged {len(values)} row(s), {df['EVENT_KEY'].nunique()} distinct event(s).")
        ...
        cursor.execute(f"""
            ...
            WHERE EVENT_KEY IN (SELECT DISTINCT EVENT_KEY FROM {STAGING_TABLE})
        """)
        print(f"   [v2] 🗑️ Deleted {cursor.rowcount} existing row(s) for events being replaced.")
        ...
        """, (run_id,))
        print(f"   [v2] ✅ Inserted {cursor.rowcount} fresh row(s) into {TARGET_TABLE}.")
    finally:
        cursor.close()
...
def main():
    ...
    if not raw_file:
        print("❌ [v2] RAW_FILE env var not set.")
        sys.exit(1)

    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    from cleaner import clean_raw_workbook_v2

    cleaned_df = clean_raw_workbook_v2(raw_file)
    if cleaned_df.empty:
        print("   [v2] ⚠️ No rows survived cleaning — nothing to load.")
        return
